...
INSERT INTO link_type (link_type_id, parent_link_type_id, name, notes) VALUES
    ...
    ('ba0ba000-0000-0000-0000-000000000018', 'ba0ba000-0000-0000-0000-000000000002',
     'TERM_SIMILAR_PROPOSED',
     'Provisional TERM_SIMILAR candidate generated by the MCP agent, pending editorial review. Used for cross-graph domain mappings where full equivalence (SAME_AS) overstates certainty (e.g. Gravity L1 social-risk domain to OE L1 service category). Follows the same propose/approve workflow: DESTROY + CREATE TERM_SIMILAR on acceptance, DESTROY alone on rejection. Set link_provenance=''proposed'' and include agent_rationale in metadata.')
ON CONFLICT (name) DO NOTHING;
...
WHERE name IN ('SAME_AS', 'SAME_AS_PROPOSED', 'TERM_EQUIVALENT', 'TERM_EQUIVALENT_PROPOSED', 'TERM_SIMILAR', 'TERM_SIMILAR_PROPOSED', 'TERM_VARIANT');
...
INSERT INTO agent_memory (memory_id, category, key, content, created_at, updated_at) VALUES
    ...
    (gen_random_uuid(), 'general', 'meta:bql-limit-override',
     ...
     CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
    (gen_random_uuid(), 'general', 'meta:crosswalk-link-metadata',
     '[ALWAYS] Crosswalk link metadata canonical keys: confidence_level (HIGH | MEDIUM | LOW | UNCERTAIN) | confidence_basis (list from: name_similarity | code_overlap | domain_alignment | population_overlap | service_alignment | no_clear_signal) | agent_rationale (string: key evidence for the match) | caveats (string or null: scope limitations or reasons against) | link_provenance (proposed | asserted | derived). Use these exact key names - no variations or additions.',
     CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
